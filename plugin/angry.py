...
OVERUSED_INTRO = {'However'            :  'But or Yet'           ,
...
def abbreviations(text):
    ...
    # Check how often each abbreviation occurs and
    # comment if less than five
    found_abbreviations = []
    ...
    if len(found_abbreviations) == 1:
        mistakes.append(
            f'The abbreviation {found_abbreviations[0]} occurs only a few times.'
        )
    if len(found_abbreviations) > 1:
        ...
        for name in found_abbreviations[1:]:
            output_string += f', {name}'
        mistakes.append(
            f'The abbreviations {output_string} occur only a few times each'
        )
    return mistakes
...
def references(text):
    ...
    # Analyse the age of the references and comment
    # if more than 40% are old
    years = []
    try:
...
def line_is_valid(line):
    '''排除注释 Check if the line is not empty and not a Latex comment'''
    validation = False
    line = line.lstrip()
    if len(line) > 1:
        if line[0] != '%':
